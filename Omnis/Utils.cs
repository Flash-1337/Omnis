...
namespace Omnis
{
    internal class Utils
    {
        ...
        public static void Inject(string filePath)
        {
            ...
            IntPtr allocMemAddress = VirtualAllocEx(procHandle, IntPtr.Zero, (uint)((filePath.Length + 1) * Marshal.SizeOf(typeof(char))), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

            UIntPtr bytesWritten;
            WriteProcessMemory(procHandle, allocMemAddress, Encoding.Default.GetBytes(filePath), (uint)((filePath.Length + 1) * Marshal.SizeOf(typeof(char))), out bytesWritten);
